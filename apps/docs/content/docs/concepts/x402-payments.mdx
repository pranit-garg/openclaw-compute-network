---
title: x402 Payments
description: How Dispatch uses Coinbase's x402 protocol for HTTP-native stablecoin micropayments on every job.
---

# x402 Payments

Dispatch uses [x402](https://www.x402.org/) — Coinbase's open HTTP-native payment protocol — to settle stablecoin micropayments on every compute job. No custom token, no staking, no bridging. Workers get paid in USDC the moment a job completes.
