---
title: Job Lifecycle
description: How jobs flow through the OpenClaw network — from submission to completion, with every status transition explained.
---

# Job Lifecycle

Every job in OpenClaw moves through a defined set of statuses. Understanding this lifecycle is key to building reliable integrations.
...
These are defined in the `JobStatus` enum in `@openclaw/protocol`:
