...
The `@dispatch/compute-router` package is the main SDK for submitting jobs to the Dispatch network. It routes jobs to decentralized workers first, then falls back to hosted BYOK providers (OpenAI, Anthropic) if the decentralized network is unavailable.
